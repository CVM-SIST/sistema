...
class Comisiones_model extends CI_Model {
    ...
    public function resumen($comision, $periodo, $anio_corte,$suspendido) {
	...
	$qry = "CREATE TEMPORARY TABLE tmp_socios ( INDEX (sid) )
		...
		FROM actividades_asociadas aa 
			JOIN actividades a ON ( a.comision = $comision AND a.id = aa.aid )
			JOIN socios s ON ( aa.sid = s.Id AND s.suspendido = $suspendido ); ";
        $this->db->query($qry);
